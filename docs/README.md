...
| Get a new repo info to create it (server-side) | POST | functionality | {"request": "server_get_job_info", "server_name": <server_name>, "server_password": <server_password>, "repo_id": <repo_ID>} | / | {"code": <code_code>, "message": <response_message>, "payload": <payload>} | Usefull for the server to ask a repo info to create it |
